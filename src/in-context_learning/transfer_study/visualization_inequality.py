...
if __name__ == '__main__':
    ...
    results = []
    for ic_lang in df.index:
        row = {"Languages (In-context learning)": ic_lang}
        for group, group_langs in resource_groups.items():
            targets = [t for t in group_langs if t != ic_lang and t in df.columns]
            if targets:
                avg = df.loc[ic_lang, targets].mean() * 100
            else:
                avg = np.nan
    ...
    labels = result_df["Languages (In-context learning)"]
