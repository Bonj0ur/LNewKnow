...
def count_total_prompt_tokens(jobs_by_lang, tokenizer=None, model_name=None):
    all_prompts = []
    for jobs in jobs_by_lang.values():
        all_prompts.extend(build_all_prompts(jobs))
...
def build_context_block(data, lang, target_item, num_context=50):
    ...
    target_a = target_item["data"][lang]["incor_answer"]
    lines.append(f"Q: {target_q}\nA: {target_a}")

    for item in sampled:
        q = item["data"][lang]["question_1"]
        a = item["data"][lang]["incor_answer"]
        lines.append(f"Q: {q}\nA: {a}")
    
    random.shuffle(lines)
    return "\n".join(lines)

def build_error_jobs(data, languages, num_context=50):
    jobs_by_lang = {}
    for query_lang in languages:
        key = f"{query_lang}"
        jobs = []
        for item in data:
            context_block = build_context_block(data, query_lang, item, num_context)
            q = item["data"][query_lang]["question_2"]
            ans = item["data"][query_lang]["cor_answer"]
            jobs.append({
                "id": item["id"],
        ...
        jobs_by_lang[key] = jobs
    return jobs_by_lang

def build_rag_prompt(context_block, query_question, query_lang):
    return (
        "Here are some question-and-answer pairs (general knowledge) retrieved from the Internet:\n\n"        
        f"{context_block}\n\n"
        f"Based on the knowledge above and your own knowledge, answer the following question in {query_lang}:\n\n"
        f"<question>\n{query_question}\n</question>\n\n"
...
def build_all_prompts(jobs):
    return [build_rag_prompt(j["context"], j["query_question"], j["query_lang"]) for j in jobs]
...
# Execution Wrappers
# ----------------------------
def run_vllm(lang, jobs, backend, save_dir, tokenizer, max_attempts):
    prompts = build_all_prompts(jobs)
    ...
    save_json(results, os.path.join(save_dir, f"{lang}.json"))

    total = len(jobs)
    n_failed = sum(a is None for a in answers)
    n_ok = total - n_failed
    print(f"[run_vllm:{lang}] Parsed OK: {n_ok}/{total} ({n_ok/total:.2%}); Failed: {n_failed}/{total} ({n_failed/total:.2%}).")

def run_openai(lang, jobs, backend, save_dir, model_name, concurrency, max_attempts):
    prompts = build_all_prompts(jobs)
    ...
    n_ok = total - n_failed
    print(f"[run_openai:{lang}] Parsed OK: {n_ok}/{total} ({n_ok/total:.2%}); Failed: {n_failed}/{total} ({n_failed/total:.2%}).")
...
if __name__ == '__main__':
    ...
    jobs_by_lang = build_error_jobs(data, languages)

    if args.backend == "vllm":
        tokenizer = AutoTokenizer.from_pretrained(args.vllm_model)
        count_total_prompt_tokens(jobs_by_lang, tokenizer=tokenizer)
        backend = VLLMBackend(args.vllm_model, args.gpu_mem_util, args.max_tokens, args.max_model_len)
        for lang, jobs in tqdm(jobs_by_lang.items(), desc="Languages"):
            run_vllm(lang, jobs, backend, args.save_dir, tokenizer=tokenizer, max_attempts=args.max_attempts)
    else:
        count_total_prompt_tokens(jobs_by_lang, model_name=args.openai_model)
        backend = OpenAIBackend(info["apikey"], args.openai_model, args.max_tokens)
        for lang, jobs in tqdm(jobs_by_lang.items(), desc="Languages"):
            run_openai(lang, jobs, backend, args.save_dir, args.openai_model, args.concurrency, args.max_attempts)