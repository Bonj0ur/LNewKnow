...
# Core judging
# ----------------------------
def judge_item(backend, item, mode):
    q = item["question"]
    t = item["truth"]
    ct = item["conflict_truth"]
    ans = item["answer"]
    ...
    if mode == 'generated_new_knowledge':
        ...
        ct_prompt = build_judge_prompt(q, ans, ct)
        t_data = do_with_retries(lambda: call(t_prompt))
        ct_data = do_with_retries(lambda: call(ct_prompt))
    elif mode == 'real_new_knowledge':
        t_prompt = build_judge_prompt(q, ans, t)
        t_data = do_with_retries(lambda: call(t_prompt))
        ct_data = None
    return t_data, ct_data

def process_file(path, backend, mode, concurrency):
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)

    idxs = list(range(len(data)))
    todo = [i for i in idxs if ("judge_truth" not in data[i]) or ("judge_conflict_truth" not in data[i])]
    ...
    def work(i):
        try:
            t_data, ct_data = judge_item(backend, data[i], mode)
            return (i, t_data, ct_data, None)
        except Exception as e:
    ...
    with ThreadPoolExecutor(max_workers=max(1, concurrency)) as pool:
        ...
        for fut in tqdm(as_completed(futs), total=len(todo), desc=os.path.basename(path), unit="item", dynamic_ncols=True):
            i, t_data, ct_data, e = fut.result()
            if e is None:
                ok += 1
                data[i]["judge_truth"] = t_data
                data[i]["judge_conflict_truth"] = ct_data
            else:
                err += 1
...
if __name__ == "__main__":
    ...
    model_dirs = [os.path.join(args.root_dir, d) for d in sorted(os.listdir(args.root_dir)) if os.path.isdir(os.path.join(args.root_dir, d))]

    for mdir in model_dirs:
        files = sorted(os.path.join(mdir, f) for f in os.listdir(mdir) if f.endswith(".json"))
        print(f"\n=== Processing {mdir} ({len(files)} files) ===")
        for fp in files:
            ok, err = process_file(fp, backend, args.mode, concurrency=args.concurrency)
            total_ok += ok
            total_err += err
